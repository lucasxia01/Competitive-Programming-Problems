...
int main() {
    ...
    int n; cin >> n;
    string s; cin >> s;
    stringHash SH1(101, 1e9+7, s);
    stringHash SH2(37, 1e9+9, s);
    const int SQRTN = 200;
    int dp[n][SQRTN]; F0R(i, n) F0R(j, SQRTN) dp[i][j] = 0;
    F0R(i, n) {
        F0R(j, SQRTN) {
            // find next occurrence of s[i, i+j] in s after i+j
            FOR(k, i+j+1, n-1-j) {
                // check if s[i, i+j] == s[k, k+j]
                if (SH1.getHash(i, i+j) == SH1.getHash(k, k+j) && SH2.getHash(i, i+j) == SH2.getHash(k, k+j)) {
                    ckmax(dp[k][])
                    break;
                }
            }
    ...
    int ans = 0;
    F0R(i, n) F0R(j, SQRTN) ckmax(ans, dp[i][j]);
    cout << ans << nl;
    return 0;
}
