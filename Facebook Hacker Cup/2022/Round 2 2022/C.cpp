...
void solve() {
    ...
    int x = 0, y = 0, m = 0;
    F0R(i, n) {
        if (a[i].s == a[0].s) x += a[i].f;
        if (a[i].s > a[0].s) y += a[i].f;
        m += a[i].f;
    }
    ll num = mod(cmb.comb(m-y, k+1) - cmb.comb(m-x-y, k+1));
    ll ans = mod(num * cmb.mod_inv(cmb.comb(m, k+1)));
    ans = mod(ans*mod(a[0].f*cmb.mod_inv(x)));
    cout << ans << nl;
}
