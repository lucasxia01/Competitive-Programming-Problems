...
Mint sum_squares(Mint x) {
    return x*(x+1)*(Mint(2)*x+1) / Mint(6);
}
signed main() {
    ...
    while (t--) {
        ...
        Mint A = a;
        Mint B = b;
        // find the largest square under a
        Mint x = find_sqrt(a);
        // find the sum of these squares
        Mint ans = sum_squares(x);
        ans += x;
        // for between A and B
        Mint y = find_sqrt(b);
        // we add a+1 for each one
        ans += (A+1)*(y-x);
        // for between B and A+B
        Mint z = find_sqrt(a+b);
        // we add A+B+1-i for each one, where i is the sq
        ans += (A+B+1)*(z-y);
        ans -= sum_squares(z)-sum_squares(y);
        cout << ans+1 << nl;
    }
    return 0;
}
