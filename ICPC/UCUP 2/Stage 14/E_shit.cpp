        ...
        while (!q.empty()) {
            ...
            trav(v, gr[neighbor]) {
                if (!dead[v]) {
                    degcnt[v]--;
                    if (degcnt[v] == 2) {
                        trav(uu, gr[v]) if (degcnt[uu] == 1 && !dead[uu]) {
                            q.push(uu);
                        }
                    }
                    if (degcnt[v] == 1 && !dead[v]) {
                        // check if it connects to a degree 2 thing first
                        bool ok = 0;
                        trav(uu, gr[v]) if (degcnt[uu] == 2 && !dead[uu]) {
                            q.push(v);
                            ok = 1;
                        }
                        leaves.push(v);
        ...
        while (!leaves.empty()) {
            ...
            if (!dead[l]) {
                ...
                trav(u, gr[l]) {
                    if (!dead[u]) {
                        degcnt[u]--;
                        dbg("deleting edge to", u, "with new degree", degcnt[u]);
                        if (degcnt[u] == 2) {
                            trav(v, gr[u]) {
                                dbg("looking at", v, "with degree", degcnt[v], "and dead", dead[v]);
                                if (degcnt[v] == 1 && !dead[v]) {
                                    q.push(v);
                                }
                            }
                        }
                        if (degcnt[u] == 1 && !dead[u]) {
                            // check if it connects to a degree 2 thing first
                            bool ok = 0;
                            trav(uu, gr[u]) if (degcnt[uu] == 2 && !dead[uu]) {
                                q.push(u);
                                ok = 1;
                            }
                            leaves.push(u);
