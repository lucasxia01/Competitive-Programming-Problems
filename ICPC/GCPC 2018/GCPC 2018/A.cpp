...
int main() {
    ...
    string s;
    getline(cin, s);
    string m[h+1]; F0R(i, h+1) getline(cin, m[i]);
    FOR(i, 1, h) F0R(j, w) {
        int u = pt2node(i, j);
        // above
        if (m[i-1][2*j+1] != '_') gr[u].pb(pt2node(i-1, j));
        if (m[i][2*j+1] != '_') gr[u].pb(pt2node(i+1, j));
        if (m[i][2*j] != '|') gr[u].pb(pt2node(i, j-1));
        if (m[i][2*j+2] != '|') gr[u].pb(pt2node(i, j+1));
    }
    preprocess();
    int q, u, v;
    cin >> q;
    ...
    ll ans = 0;
    F0R(i, q-1) {
        ans += l[p[i]] + l[p[i+1]] - 2*l[lca(p[i], p[i+1])];
    }
    cout << ans << nl;
    return 0;
}
