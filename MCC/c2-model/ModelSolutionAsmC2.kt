...
fun program(): Cassette = buildCode {
    ...
    label("loadinput")
    op(LoadOp(Register.RD, Expression(null, (Memory.INPUT_BEGIN + 0).toByteSafe())))
    ret()

    label("run")
    call("loadinput")
    op(BinaryOp(OperationType.Add, Register.RA, Register.RD, Register.RD))
    ...
    op(SetOp(Register.RE, Expression(null, 'A'.code.toByteSafe())))
    label("inner")
    ...
    op(SetOp(Register.RG, Expression(Register.RE, (-'D'.code).toByteSafe())))
    ...
    op(
        SetOp(
            Register.RA, Expression(
                Register.RINS,
                (getLabel("#head").toInt() - address().toInt() - 4).toByteSafe()
            )
        )
    )
    ...
    label("check")
    op(SetOp(Register.RF, Expression(null, 1)))

    op(DebugOp(tag = 30u))
    op(
        SetOp(
            Register.RD, Expression(
                Register.RINS,
                (getLabel("#begin").toInt() - address().toInt() - 4).toByteSafe()
            )
        )
    )
    op(
        SetOp(
            Register.RE, Expression(
                Register.RINS,
                (getLabel("#head").toInt() - address().toInt() - 4).toByteSafe()
            )
        )
    ...
    op(BinaryOp(OperationType.Sub, Register.RD, Register.RD, Register.RE))
    ...
    label("go")
    op(LoadOp(Register.RC, Expression(Register.RD)))
    op(SetOp(Register.RD, Expression(Register.RD, 4)))

    op(SetOp(Register.RC, Expression(Register.RC, (-'='.code).toByte())))
    op(SetOp(Register.RG, Expression(null, 4)))
    op(BinaryOp(OperationType.Mult, Register.RG, Register.RC, Register.RG))
    op(BinaryOp(OperationType.Add, Register.RG, Register.RINS, Register.RG))
    op(LoadOp(Register.RC, Expression(Register.RG)))
    op(StoreOp(Register.RC, Expression(Register.RINS)))

    op(DebugOp(tag = 37u))

    op(SetOp(Register.RINS, Expression(Register.RINS, 16)))
    op(SetOp(Register.RA, Expression(null, -1)))
    op(SetOp(Register.RA, Expression(null, 1)))
    op(SetOp(Register.RF, Expression(null, 0)))
    op(SetOp(Register.RF, Expression(null, 37)))

    op(DebugOp(tag = 38u))

    label("rinse.repeat")
    op(BinaryOp(OperationType.Sub, Register.RC, Register.RD, Register.RE))
    jumpIf(JumpCondition.ZERO, "go.bad")

    op(LoadOp(Register.RC, Expression(Register.RD)))

    op(SetOp(Register.RC, Expression(Register.RC, (-'C'.code).toByteSafe())))
    jumpIf(JumpCondition.ZERO, "go.fini")
    callPreservingRegisters("go", Register.RA)
    op(BinaryOp(OperationType.Add, Register.RA, Register.RA, Register.RR))
    jump("rinse.repeat")

    label("go.fini")
    op(SetOp(Register.RD, Expression(Register.RD, 4)))
    op(SetOp(Register.RA, Expression(Register.RA, 1)))
    op(SetOp(Register.RA, Expression(Register.RA, -3)))
    op(JumpOp(JumpCondition.CARRY, Expression(Register.RINS, 4)))
    op(SetOp(Register.RF, Expression(null, 0)))

    op(SetOp(Register.RR, Expression(Register.RA, +2)))
    ret()
    label("go.bad")
    op(SetOp(Register.RF, Expression(null, 0)))
    ret()
...
fun testProgram(): Cassette = buildCode {
    op(LoadOp(Register.RA, Expression(null, (Memory.INPUT_BEGIN + 0).toByteSafe())))
    call("sumall")
    op(HaltOp(Expression(Register.RR)))
...
fun input(n: UInt): Cassette = buildData {
    int(n)
}
