...
signed main() {
    ...
    while (t--) {
        ...
        int hi = 0; hi = *max_element(all(a));
        // try checking if the answer can be i
        int ans = 1;
        FORd(i, 2, n) {
            // we're adding i
            // first find the highest val that makes sum+val 0 mod i
            int x = ((-sum)%i + i)%i;
            if (x > k) continue;
            int val = ((k-x) / i)*i + x;
            assert((sum+val)%i == 0);
            if (hi*(i-1) <= (sum-hi+val)) {
                dbg(sum, hi, x, val, i-1);
                ans = i;
                break;
            }
        }
